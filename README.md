...
- **Jupyter / Windows 相容**：自動偵測事件迴圈是否支援 Playwright 子程序；不支援時改在獨立執行緒以 `ProactorEventLoop` 執行（見 `core/playwright_engine.py`）
- **品質過濾**：付費牆／不完整正文自動略過；依 URL 與標題+正文指紋去重
- **刊出時間**：統一為 `YYYY-MM-DD-HH:MM`（例：`2026-05-23-09:43`）
...
├── core/
│   ├── base_scraper.py      # 正文清洗、刊出時間格式、付費牆、記者擷取
│   └── playwright_engine.py # Playwright 下載、共用 session、Jupyter 相容層
├── scrapers/
│   ├── factory.py           # URL → 解析器
...
```mermaid
flowchart LR
    A[main_crawl_all / run_main_crawl_all] --> B{playwright 子程序支援?}
    B -->|否 Windows Jupyter| C[獨立執行緒 + ProactorLoop]
    B -->|是| D[discovery 探索列表頁]
    C --> D
    D --> E[文章內頁 URL 列表]
    E --> F[playwright_session]
    F --> G[fetch_html]
    G --> H[ScraperFactory.parse]
    H --> I[去重 + DataFrame]
    I --> J[data/*.csv]
```
...
| 函式 | 類型 | 適用環境 | 說明 |
|------|------|----------|------|
| `main_crawl_all(articles_per_category=…)` | `async` | 腳本（`asyncio.run`）、**Jupyter（`await`）** | 四家 × 七類批次爬取；**內建 Windows Jupyter 相容** |
| `run_main_crawl_all(articles_per_category=…)` | 同步 | 腳本、Jupyter（免 `await`） | 同上；Notebook 內以執行緒 + 獨立事件迴圈執行 |
...
在 **Windows** 上，Jupyter 常用 `SelectorEventLoop`，無法啟動 Playwright 子程序，可能出現 **`NotImplementedError`**（常發生在 `discovery` 探索列表頁階段）。程式已透過 `run_playwright_compatible()` 自動偵測，改在**獨立執行緒**以 `WindowsProactorEventLoopPolicy` 執行整段爬取。日誌若出現「改在獨立執行緒執行…」屬正常現象。

**修改程式後請重啟 Kernel**，避免舊版 `main` 模組仍留在記憶體。

```python
from main import main_crawl_all, run_main_crawl_all

# 方式一（建議）：直接 await（已內建 Windows/Jupyter 相容）
await main_crawl_all()
await main_crawl_all(articles_per_category=60)

# 方式二：同步包裝（不需 await，效果相同）
run_main_crawl_all()
run_main_crawl_all(articles_per_category=60)
```
...
| 錯誤 | 原因 | 解法 |
|------|------|------|
| `RuntimeError: asyncio.run() cannot be called from a running event loop` | Notebook 已有事件迴圈 | 改用 `await main_crawl_all(...)` 或 `run_main_crawl_all(...)` |
| `NotImplementedError`（探索列表頁時） | Windows Jupyter 的 `SelectorEventLoop` 無法建立 Playwright 子程序 | 新版已自動改執行緒；或手動呼叫 `run_main_crawl_all(...)`；**重啟 Kernel** 後再試 |

實作位置（`core/playwright_engine.py`）：

| 函式 | 用途 |
|------|------|
| `playwright_subprocess_supported()` | 判斷目前事件迴圈能否跑 Playwright |
| `run_playwright_compatible(factory)` | `await main_crawl_all()` 時自動選擇直接執行或改執行緒 |
| `run_async_in_playwright_loop(factory)` | 在獨立 Proactor 迴圈中執行整段非同步爬取 |

`main_crawl_all` 內部實際邏輯在 `_main_crawl_all_impl()`；`run_main_crawl_all()` 與 `await main_crawl_all()` 最終皆呼叫同一套流程。
...
`run_main_crawl_all` 為 `main_crawl_all` 的同步包裝，兩者共用 `_main_crawl_all_impl()`，並皆支援 `articles_per_category` 參數。
...
- **`NotImplementedError`**（Windows Jupyter）→ 請更新至最新程式碼並**重啟 Kernel**；仍失敗則改 `run_main_crawl_all(articles_per_category=…)`。
- 修改 `main.py` 後行為未變 → Kernel 可能仍載入舊模組，請 **Restart Kernel** 再執行。

---

## 注意事項

- **Jupyter / Windows**：批次爬取用 `await main_crawl_all(...)` 或 `run_main_crawl_all(...)`，勿用 `asyncio.run()`。Playwright 相容層見上一節；範例見 `utilities.ipynb`。
